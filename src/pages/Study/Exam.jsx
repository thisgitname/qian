...
export default function Exam() {
    ...
    const { course, page, cate, title } = location.state || {};//获取课程和页码
    ...
    // 获取当前考试的存储键
    const getStorageKey = () => {
        return `examTime_${course}_${page}`;
    };
    ...
    const getCourseTotalTime = () => {
        let totalSeconds = 0;
        // 遍历localStorage，找到所有属于当前课程的考试时间
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key.startsWith(`examTime_${course}_`)) {
    ...
    const handleBack = useCallback(() => {
        ...
        Modal.confirm({
            ...
            onConfirm: () => {
                // 保存当前考试的时间
                const currentExamTime = getTotalExamTime();
                localStorage.setItem(getStorageKey(), currentExamTime);
                ...
                navigate("/test", { 
                    state: { 
                        course, 
                        examTime: courseTotalTime // 传递课程的总考试时间
                    } 
                });
            },
            onCancel: () => {
        ...
        if (current < total) {
            ...
            if (unansweredCount > 0) {
                Modal.confirm({
                    ...
                    onConfirm: async () => {
                        // 将未答题的题目标记为未作答
                        const currentAnsweredIds = answers.map(a => a.questionId);
                        const remainingQuestions = list.filter(q => !currentAnsweredIds.includes(q._id));
                        ...
                        const examRecord = {
                            ...
                            questions: list.map((question, index) => ({
                                ...
                                userAnswer: judgedAnswers[index]?.userAnswer || null,
                                isCorrect: judgedAnswers[index]?.isCorrect || false,
                            })),
                            score,
                            total,
                            correctCount,
                        ...
                        // 获取现有的考试记录
                        const existingRecords = JSON.parse(localStorage.getItem('examRecords') || '[]');
                        
                        // 添加新记录
                        existingRecords.push(examRecord);
                        
                        // 保存回localStorage
                        localStorage.setItem('examRecords', JSON.stringify(existingRecords));
                    ...
                    onCancel: () => {
                        // 跳转到第一个未完成的题目
                        const firstUnansweredIndex = list.findIndex((_, index) => 
                            !answers.some(a => a.questionId === list[index]._id)
                        );
